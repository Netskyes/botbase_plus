...
LPVOID WSASocketWTramp;
LPVOID WSASocketATramp;
LPVOID WSAConnectATramp;
LPVOID WSAConnectWTramp;
LPVOID WSAAcceptTramp;


void __stdcall PatchSockAddr()
{
	std::cout << "Attempt to patch sock address!" << std::endl;
	...
	*sockAddrNew = *sockAddrBackup;
	...
	std::cout << "Patching sock addr: " << hostname << " ";
...
void __stdcall SendPatch(SOCKET s)
{
	...
	int resultSend;
	std::cout << "Send patch for socket: " << s << " --> " << sockInfo.iProtocol <<  std::endl;
...
void __stdcall mWSARecv(SOCKET s, LPWSABUF lpBuffers)
{
	...
	std::stringstream ss;
	ss << "Network:WSARecv -> Socket: " << s << " P: " << sockInfo.iProtocol << " T: " << sockInfo.iSocketType << " AF: " << sockInfo.iAddressFamily << "Len: " << lpBuffers->len << std::endl;
...
void __stdcall mWSASend(SOCKET s, LPWSABUF lpBuffers)
{
	...
	std::stringstream ss;
	ss << "Network:WSASend -> Socket: " << s << " P: " << sockInfo.iProtocol << " T: " << sockInfo.iSocketType << " AF: " << sockInfo.iAddressFamily << "Len: " << lpBuffers->len << std::endl;
...
__declspec(naked) int hookSocketW()
{
	__asm
	{
		pushad;
...
__declspec(naked) int hookSocketA()
{
	__asm
	{
		pushad;
...
void mHookWSAConnectByName(SOCKET s)
{
	std::cout << "WSAConnectByName: " << s << std::endl;
}

__declspec(naked) int hookWSAConnectByNameA()
{
	__asm
	{
		pushad;
		...
		push[esp + 0x1C];
		call mHookWSAConnectByName;

		popfd;
		popad;

		jmp WSAConnectATramp;
	}
}

__declspec(naked) int hookWSAConnectByNameW()
{
	__asm
	{
		pushad;
...
void HookUp()
{
	...
	//HookFunc(L"ws2_32.dll", "WSAConnectByNameA", (LPVOID*)hookWSAConnectByNameA, &WSAConnectATramp, 5);
	//HookFunc(L"ws2_32.dll", "WSAConnectByNameW", (LPVOID*)hookWSAConnectByNameW, &WSAConnectWTramp, 5);
	...
	//HookFunc(L"ws2_32.dll", "WSASocketW", (LPVOID*)hookSocketW, &WSASocketWTramp, 5);
	//HookFunc(L"ws2_32.dll", "WSASocketA", (LPVOID*)hookSocketA, &WSASocketATramp, 5);
